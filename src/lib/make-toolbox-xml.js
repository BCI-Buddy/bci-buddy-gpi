...
//this is just a copy of motion for now
const BCI = function (isInitialSetup, isStage, targetId, colors) {
    const stageSelected = ScratchBlocks.ScratchMsgs.translate(
        'MOTION_STAGE_SELECTED',
        'Stage selected: no motion blocks'
    );
    // Note: the category's secondaryColour matches up with the blocks' tertiary color, both used for border color.
    return `
    <category name="BCI" id="BCI" colour="${colors.primary}" secondaryColour="${colors.tertiary}">
        ${isStage ? `
        <label text="${stageSelected}"></label>
        ` : `
        <block type="connect_to_amp">
...
const makeToolboxXML = function (isInitialSetup, isStage = true, targetId, categoriesXML = [],
    ...
    const BCIXML = moveCategory('BCI') || BCI(isInitialSetup, isStage, targetId, colors.bci);
